...
function excuse(){
    var r = Math.floor(Math.random()*excuses[1].length);
    var r2 = Math.floor(Math.random()*excuses[0].length);
    var decide = Math.round(Math.random()*1);

    if (decide==0) {
    $('.finalexcuseenough').html('<h4>"' + excuses[0][r2] + '"</h4>');
    responsiveVoice.speak(excuses[0][r2], "UK English Male", {rate:1});
    console.log("works");
    } else {
    $('.finalexcuseenough').html('<h4>"' + excuses[1][r] + '"</h4>');
    responsiveVoice.speak(excuses[1][r], "UK English Male", {rate:1});
    console.log("works");
    }
